...
export default class FlatListDemo extends Component<Props> {
    ...
    loadData=()=>{//根据传入数据判断是上拉还是下拉

        console.log('30---------loadData:'+this.state.isFootLoading+" : "+this.state.noMoreData);
        if(this.state.isFootLoading || this.state.noMoreData) {
        ...
        setTimeout(() => {
            ...
            this.setState({
                ...
                isFootLoading: false,//把加载状态设置为不加载（即加载结束）
                noMoreData:true
            })
        }, 2000);
        console.log('49---------loadData:'+this.state.isLoading+" : "+this.state.noMoreData);
    };
        ...
        return (
            <View style={styles.container}>
                <FlatList
                    ...
                    //         onRefresh={() => {                  //触动刷新的方法
                    //             this.pullDownRefresh(true)//加载数据(带参数)
                    //         }}
                    //     />
                    // }

                    //contentContainerStyle={{backgroundColor: 'blue', height: 200}}
                    //3:自定义的上拉加载数据
